...
import { useChargesExtract } from "./hooks/useChargesExtract";
import { FinancesStructProps } from "../type";
import { Selector } from "@components/shared/layouts/Seletor";
import i18n from "@configs/i18n";

export function ChargesExtract({ filterObjects, search }: FinancesStructProps) {
  const { selectors, setSelectors, tHeadsFinance } = useChargesExtract({
    filter: search,
    ...
    <div className="mt-6">
      <SelectorProvider selectors={selectors} setSelectors={setSelectors}>
        <SmartTable
          ...
          data={[]}
          title={i18n("words.charges_extracts")}
